frappe.ui.form.on('Purchase Import', {
    ...
    refresh(frm) {
        ...
        frm.trigger('format_purchase_invoice_links');

        // Register realtime listeners ONCE per form instance
        if (!frm._realtime_bound) {
            frm._realtime_bound = true;

            frappe.realtime.on('akfa_daily_purchase_log', (data) => {
                if (data.doc_name === frm.doc.name) {
                    if (data.msg.includes("TA'MINOTCHI") || data.msg.includes('YAKUNLANDI')) {
                        frappe.show_alert({ message: data.msg, indicator: 'blue' });
                    }
                }
            });

            frappe.realtime.on('akfa_daily_purchase_success', (data) => {
                if (data.doc_name === frm.doc.name) {
                    if (frm._import_poll) {
            ...
            frappe.realtime.on('akfa_daily_purchase_failed', (data) => {
                if (data.doc_name === frm.doc.name) {
                    if (frm._import_poll) {
                        clearInterval(frm._import_poll);
                        frm._import_poll = null;
    ...
    auto_set_warehouse(frm) {
        ...
        frappe.call({
            method: 'akfa_diller.akfa_diller.api.daily_purchase_import.get_default_warehouse',
    ...
    validate_prerequisites(frm) {
        ...
        frm.set_df_property('excel_file', 'description', valid
            ? 'Kunlik xarid Excel fayli (.xlsx)'
            : '<span style="color:red;">⚠️ Avval Компания, Ombor, Cost Center va Sanani tanlang!</span>'
        );
    },

    format_purchase_invoice_links(frm) {
        if (frm.doc.status !== 'Processed') return;

        if (frm.doc.purchase_invoice) {
            const pi_links = frm.doc.purchase_invoice.split(',')
                .map(s => s.trim()).filter(s => s)
                .map(pi => `<a href="/app/purchase-invoice/${pi}">${pi}</a>`)
                .join('<br>');

            setTimeout(() => {
                const el = frm.fields_dict.purchase_invoice?.$wrapper?.find('.like-disabled-input, .control-value');
                if (el?.length) el.html(pi_links);
            }, 100);
        }
    },
    ...
    setup_buttons(frm) {
        ...
        if (frm.doc.status === 'Processed') {
            ...
            if (frm.doc.purchase_invoice) {
                const names = frm.doc.purchase_invoice.split(',').map(s => s.trim()).filter(s => s);
                frm.add_custom_button(__('Purchase Invoice ({0})', [names.length]),
                    () => frappe.set_route('List', 'Purchase Invoice', { name: ['in', names] }), __('View'));
            }
        }
    ...
    show_preview(frm) {
        frappe.call({
            method: 'akfa_diller.akfa_diller.api.daily_purchase_import.get_preview_data',
    ...
    render_preview(frm, data) {
        ...
        const rows = receipts.map((receipt) => {
            const supplier_badge = receipt.supplier_matched
                ? `<span class="badge badge-success">${frappe.utils.escape_html(receipt.supplier)}</span>`
                : `<span class="badge badge-danger">${__('topilmadi')}: ${frappe.utils.escape_html(receipt.supplier_input || '')}</span>`;

            return `<tr class="${receipt.supplier_matched ? '' : 'table-danger'}">
                <td>${receipt.date || ''}</td>
                <td>${supplier_badge}</td>
            ...
            fields: [{
                ...
                options: `
                    <div class="row mb-3">
                        ...
                        <div class="col"><div class="card bg-info text-white text-center p-2"><h4 class="mb-0">${s.total_receipts || 0}</h4><small>Purchase Invoice (Ta'minotchi+Sana)</small></div></div>
                        ...
                        <div class="col"><div class="card bg-danger text-white text-center p-2"><h4 class="mb-0">${s.suppliers_not_found || 0}</h4><small>Ta'minotchi topilmadi</small></div></div>
                    </div>
                    <p><strong>💰 Jami:</strong> ${format_currency(s.total_amount || 0, 'UZS')}</p>
                    <div class="table-responsive" style="max-height:350px;overflow-y:auto;">
                        <table class="table table-sm table-bordered">
                            <thead class="thead-dark"><tr>
                                <th>Sana</th><th>Ta'minotchi</th>
    ...
    validate_items(frm) {
        frappe.call({
            method: 'akfa_diller.akfa_diller.api.daily_purchase_import.validate_excel_items',
            args: { doc_name: frm.doc.name },
            freeze: true,
            ...
            fields: [{
                ...
                options: `
                    ...
                    <div class="alert alert-info mt-3">
                        ...
                        Har bir (Ta'minotchi, Sana) juftligi uchun ALOHIDA Purchase Invoice yaratiladi (Update Stock ON) --<br>
                        bir xil ta'minotchidan boshqa kunda xarid qilingan bo'lsa ham alohida Invoys bo'ladi.<br>
    ...
    cancel_import(frm) {
        const piCount = frm.doc.purchase_invoice ? frm.doc.purchase_invoice.split(',').filter(s => s.trim()).length : 0;
        ...
        const dlg = new frappe.ui.Dialog({
            ...
            fields: [{
                fieldtype: 'HTML',
                options: `
                    <p><strong>Bekor qilinadigan Purchase Invoice'lar (${piCount} ta):</strong></p>
                    <p>${frm.doc.purchase_invoice || 'N/A'}</p>
            ...
            primary_action() {
                ...
                frappe.call({
                    method: 'akfa_diller.akfa_diller.api.daily_purchase_import.cancel_import',
                    args: { doc_name: frm.doc.name },
                    freeze: true,
